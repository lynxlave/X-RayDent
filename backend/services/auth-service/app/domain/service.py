...
class AuthService:
    ...
    def _build_token_pair(self, user_id: str, role: Role) -> AuthToken:
        ...
        return AuthToken(access_token=access_token, refresh_token=refresh_token, role=role, user_id=user_id)
    ...
    def staff_login(self, payload: StaffLoginRequest) -> AuthToken:
        ...
        if user.locked_until and user.locked_until > now:
            ...
            raise ValueError(f"Account is temporarily locked. Try again in {remaining_seconds} seconds.")
        if user.password_hash != self._hash_password(payload.password):
            ...
            if failed_attempts >= 3:
                ...
                raise ValueError(
                    f"Account is temporarily locked for {lockout_minutes} minute(s) after repeated failed attempts."
                )
            self.repository.update_login_failure(
                user.id,
            ...
            raise ValueError(f"Invalid credentials. {attempts_left} attempt(s) left before temporary lock.")
